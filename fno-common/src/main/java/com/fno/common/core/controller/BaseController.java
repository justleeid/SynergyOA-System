package com.fno.common.core.controller;
...
import com.fno.common.utils.*;
import com.fno.common.utils.sql.SqlUtil;
...
public class BaseController
{
     ...
     * 返回成功消息
     */
    public AjaxResult success(String message,Object data,String jsonType)
    {
        if(Constants.RENDER_JSON_TYPE_ARR.equals(jsonType)){
            //这样操作会将null的值去除掉
            String jsonStr = JSONArray.toJSONString(data);
            JSONArray toJS = JSONArray.parseArray(jsonStr);
            return AjaxResult.success(message,toJS);
        }else if(Constants.RENDER_JSON_TYPE_OBJ.equals(jsonType)){
            //这样操作会将null的值去除掉
            String jsonStr = JSONObject.toJSONString(data);
            JSONObject toJS = JSONObject.parseObject(jsonStr);
            return AjaxResult.success(message,toJS);
        }else{
