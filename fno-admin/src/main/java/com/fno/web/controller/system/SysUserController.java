...
import com.fno.system.service.ISysRoleService;
...
public class SysUserController extends BaseController
{
    ...
    public AjaxResult getInfo(@PathVariable(value = "userId", required = false) Long userId)
    {
        ...
        ajax.put("posts", postService.selectPostAll(SecurityUtils.getTenantId()));
    ...
    public AjaxResult isExist(@Validated @RequestBody SysUser user){
        if (UserConstants.NOT_UNIQUE.equals(userService.checkUserNameUnique(user.getUserName(),false)))
        {
            return AjaxResult.error("账号'" + user.getUserName() + "'已经存在，不能重复");
        }
    ...
    @GetMapping("/resetAdminPwd/{adminName}")
    public AjaxResult resetAdminPwd(@PathVariable("adminName") String adminName)
    {
        return toAjax(userService.resetUserPwd(adminName,SecurityUtils.encryptPassword("123456")));
     ...
     * @return
     */
    @GetMapping("/commonQueryUserList")
    public AjaxResult commonQueryUserList(String str){
        List<SysUser> list = userService.commonQueryUserList(str);
        return success(list);
    }
