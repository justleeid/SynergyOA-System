...
@Transactional
@Service
public class FlowImgService extends BaseService {
    @Autowired
        ...
        for (int k = 0; k < finishedActivityInstanceList.size(); k++) {
            ...
            if ("parallelGateway".equals(currentActivityInstance.getActivityType()) || "inclusiveGateway".equals(
                ...
                for (int i = 0; i < historicActivityInstanceList.size(); i++) {
                    ...
                    logger.info("第【"+( i + 1)+"/"+size+"】个历史节点-ActivityId=["+historicActivityInstance.getActivityId()+"]");
                    ...
                    if (i >= k && historicActivityInstance.getActivityId().equals(currentActivityId)) {
                        logger.info("第["+(i+1)+"]个历史节点和当前节点一致-ActivityId=["+historicActivityInstance.getActivityId()+"]");
                        ifStartFind = true;
                        // 跳过当前节点继续查找下一个节点
                        continue;
                    }
                    if (ifStartFind) {
                        logger.info("[开始]-循环当前节点-ActivityId=【"+currentActivityId+"】的所有流出线");
                        ...
                        for (SequenceFlow sequenceFlow : outgoingFlowList) {
                            ...
                            if (historicActivityInstance.getActivityId().equals(sequenceFlow.getTargetRef())) {
                                logger.info("当前节点["+currentActivityId+"]需高亮显示的流出线=["+sequenceFlow.getId()+"]");
                                highLightedFlowIdList.add(sequenceFlow.getId());
                        ...
                        logger.info("[完成]-循环当前节点-ActivityId=【"+currentActivityId+"】的所有流出线");
                    }
                    if (ifFinded) {
                ...
                logger.info("【完成】-匹配当前节点-ActivityId=【"+currentActivityId+"】需要高亮显示的流出线");
     ...
     * @throws Exception
     */
    public String generateImageByProcInstId(String procInstId) throws Exception {
        ...
        try {
            ...
            return Base64Util.byte2Base64(buffer);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("通过流程实例ID["+procInstId+"]获取流程图时出现异常！", e);
     ...
     * @throws IOException
     */
    public String getDefImgByInsId(String insId) throws IOException {
        String defId = historyService.createHistoricProcessInstanceQuery().processInstanceId(insId).singleResult().getProcessDefinitionId();
        ProcessDefinition def = repositoryService.createProcessDefinitionQuery().processDefinitionId(defId).singleResult();
        return actReProcdefService.readResourceImg(defId,def.getDiagramResourceName());
    }
    ...
    public String getDefImgByBusinessId(Long businessId,String billType) throws IOException {
        ...
        if(list!=null && list.size()>0){
            String insId = list.get(0).getId();
            imgStr = flowImgService.getDefImgByInsId(insId);
        }
        return imgStr;
    }
}
